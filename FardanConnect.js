...
const dtod = "6285746030186@s.whatsapp.net"
const otod = "6285746030186@s.whatsapp.net"
...
try {
		...
		const katalog = (teks) => {
             res = Fardan.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© MoonBot_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
...
Arigatou Gozaimasu! Untuk kalian user ramah dan Beberapa orang yg ikut membantu juga dalam project pembuatan MoonBot-
😖🙏

For GitHub : https://github.com/FaxBotz
YouTube Fardan : https://youtube.com/c/FaxBotz
`
sendButLocation(from, `${menu}`, "MoonBot", {jpegThumbnail:ofrply,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
case 'menu':
        case 'help':
        const ptod = "6285746030186@s.whatsapp.net"
...
${yes}Creator by Rull${yes}
       
       
       
       `

sendButDocument(from, `${menu}`, "Cretaed Rull\nGunakan Bot Dengan Sebaik Mungkin!", fs.readFileSync('./Fardan.jpg'), {mimetype:Mimetype.pdf, thumbnail:ofrply, filename:`Fax Botz`}, [{buttonId:`${prefix}allmenu`,buttonText:{displayText:'COMMAND'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'OWNER'},type:1},{buttonId:`${prefix}sewabot`,buttonText:{displayText:'PRESINCE'},type:1}], {quoted:fmen, contextInfo: { mentionedJid: [dtod,otod,ptod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`Hai Kak ${pushname}`,body:`${calender} / ${timeWib}`,mediaType:"2",thumbnail:ofrply,mediaUrl:`https://urlis.net/Moon`}}})
break
case 'bugreport':
              if (args.length < 1) return reply(`Ketik ${prefix}bugreport [fiturnya] [Error Nya Gimana]`) 
              teks = args.join(' ')
              reply('Terima Kasih Telah Melaporkan Bug Pada Owner, Jika Itu Sekedar Iseng Maka Akan Di Ban Oleh Bot!')
              Fardan.sendMessage('6285746030186@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
              case 'verify':
...
case 'pornhub':
            ...
            reply (mess.wait)
              ...
              buttonsMessage = {footerText:'MoonBot 𝚅3.0.0', imageMessage: imageMsg,
              contentText:`MoonBit 𝙼𝚊𝚍𝚎 𝙱𝚢 Rull`,buttons,headerType:4}
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					...
					if (Number(args[0]) === 1) {
						...
						Fardan.sendMessage(from, `Moon 𝚅3.0.0`, text)
...
case 'loli':
       ...
       case 'wallml':
              ...
              buttonsMessage = {footerText:'Moon 𝚅3.0.0', imageMessage: imageMsg,
...
katalog(menu)
sendButLocation(from, `Hai ${pushname} ☺️`, "*_© MoonBot.ೃ࿐_*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}sewa`,buttonText:{displayText:'SEWA 🎟️'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
break
case 'stopjadibot':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
try {
...
buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `NEXT`},type:1}]
              imageMsg = (await Fardan.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'MoonBot 𝙼𝚊𝚍𝚎 𝙱𝚢 Rull', imageMessage: imageMsg,
              contentText:`klik Next untuk ke gambar selanjut nya`,buttons,headerType:4}
...
_Bisa Beli Script Bot WhatsApp Juga_`
				sendButLocation(from, `${memek}`, `Follow TikTok\nRullMoonn!!`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'Done 🌌'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break								            
case 'donate': 
       case 'donasi':
...
case 'logo':
                    ...
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} Moon`)
                    txt1 = args[0]
                    txt2 = args[1]
                    getBuffer(`https://api-helgazex.herokuapp.com/api/canvas/gfx3?text=${txt1}&text2=${txt2}&apikey=${HelKey}`).then((gambar) => {
                        Fardan.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Nih Kak!`, quoted : mek})
